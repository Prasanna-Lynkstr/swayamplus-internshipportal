...
import { MODELS, User, Student, StudentPreference, Employer, EmployerEoi, Internship, InternshipApplication, ApplicationNote, InternshipRequest } from '../src/database/models/index.js';

// Realistic, sizeable demo dataset — students, employers, internships
// (across all statuses/categories), and applications (across the full
// status pipeline), so a demo doesn't open on an empty platform. Every
// Employer is seeded as the product of an approved EmployerEoi — the only
// path to an employer account in the real system — with a couple of
// still-pending and rejected EmployerEoi rows left unconverted so the
// admin EOI queue has real content to demo too. Idempotent: every seeded
// user's identifier (and every EOI-only row's email) lives under
// DEMO_DOMAIN, so re-running detects prior seed data and no-ops unless
// --reset is passed (which deletes only rows under that domain, in FK
// order, before reseeding) — never touches real accounts from OTP sign-ups
// or seed-admin.ts.
...
// Every one of these becomes a real employer account — seeded as the
// product of an approved EmployerEoi, exactly like EmployerEoiService's
// real conversion, since that's now the only way an Employer row can
// exist.
...
async function main() {
  ...
  const existingDemoEois = await EmployerEoi.findAll({ where: { email: { [Op.like]: `%@${DEMO_DOMAIN}` } } });

  if (existingDemoUsers.length + existingDemoEois.length > 0 && !reset) {
    console.log(
      `Found ${existingDemoUsers.length} existing demo accounts and ${existingDemoEois.length} demo EOI rows (@${DEMO_DOMAIN}). Skipping — pass --reset to wipe and reseed.`,
    );
  ...
  if (existingDemoUsers.length + existingDemoEois.length > 0 && reset) {
    console.log(
      `--reset passed: removing ${existingDemoUsers.length} existing demo accounts and ${existingDemoEois.length} demo EOI rows…`,
    );
    ...
    // EOI rows first (they reference employer ids, not a real FK, but
    // deleting before the employers/users they may point at keeps the
    // ordering easy to reason about) — covers both converted rows and the
    // still-pending/rejected EOI-only ones, which have no User to key off.
    await EmployerEoi.destroy({ where: { email: { [Op.like]: `%@${DEMO_DOMAIN}` } } });
  ...
  console.log('Seeding employers (each as an approved EmployerEoi conversion)…');
  ...
  for (let i = 0; i < EMPLOYERS.length; i++) {
    ...
    const cin = `U${intBetween(10000, 99999)}TN2024PTC${intBetween(100000, 999999)}`;
    // Must be real internship_category taxonomy values, not employment-type/
    // work-mode ones — EoiForm.tsx's "Internship types expected" chips are
    // populated from useTaxonomy('internship_category'), and EmployersService
    // re-validates this field against that same taxonomy on every profile
    // save, not just at EOI submission time.
    const internshipTypesExpected = pickN(Object.keys(CATEGORY_TEMPLATES), 2);

    const eoi = await EmployerEoi.create({
      email,
    ...
    eoi.convertedEmployerId = employer.id;
    await eoi.save();

    employerRecords.push({ employer, seed });
  }

  console.log('Seeding still-pending and rejected EOI submissions (no account created)…');
  for (let i = 0; i < EOI_ONLY_SUBMISSIONS.length; i++) {
    const seed = EOI_ONLY_SUBMISSIONS[i];
    const createdAt = daysAgo(intBetween(1, 14));
    const decided = seed.status === 'rejected';
    await EmployerEoi.create({
      email: `eoi.${i + 1}@${DEMO_DOMAIN}`,
      organizationName: seed.orgName,
      contactPersonName: seed.contactName,
      contactPersonPhone: seed.contactPhone,
      reasonForEoi: seed.reasonForEoi,
      ...
      website: seed.website,
      hqCity: seed.hqCity,
      internshipTypesExpected: pickN(Object.keys(CATEGORY_TEMPLATES), 2),
      industryTags: seed.industryTags,
      acceptedTermsAt: createdAt,
      status: seed.status,
      decidedAt: decided ? daysAgo(intBetween(0, 13)) : null,
      decidedByAdminUserId: decided ? adminUserId : null,
      createdAt,
      updatedAt: createdAt,
    });
  }
  ...
  for (let i = 0; i < STUDENT_COUNT; i++) {
    ...
    const student = await Student.create({
      ...
      // ~1 in 9 students opts out of the employer-facing candidate directory,
      // so that path is actually demonstrable rather than vacuously true.
      discoverableToEmployers: i % 9 !== 0,
      createdAt,
      updatedAt: createdAt,
    });
    students.push(student);

    // Roughly half of complete profiles get real Preferences filled in —
    // enough to make preference-aware relevance ranking demonstrable
    // without pretending every seeded student actually did this (most
    // real users never touch an optional profile section at all).
    if (complete && i % 2 === 0) {
      await StudentPreference.create({
        studentId: student.id,
        preferredCategories: pickN(Object.keys(CATEGORY_TEMPLATES), intBetween(1, 2)),
        preferredModes: pickN(['remote', 'onsite', 'hybrid'], intBetween(1, 2)),
        preferredEmploymentTypes: pickN(['full-time', 'part-time'], 1),
        paidPreference: pick(['paid', 'paid', 'either', 'unpaid']),
        preferredLocations: pickN(CITIES, intBetween(1, 2)),
        availabilityStatus: pick(['actively_looking', 'actively_looking', 'not_looking']),
        createdAt,
        updatedAt: createdAt,
      });
    }
  ...
  for (const { employer, seed } of employerRecords) {
