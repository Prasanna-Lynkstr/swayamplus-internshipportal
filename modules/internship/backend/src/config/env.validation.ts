...
export const envValidationSchema = Joi.object({
  ...
  // Throttles the one public, unauthenticated write endpoint in this app
  // (POST /employer-eoi) — see EmployerEoiService.assertNotThrottled.
  EOI_SUBMIT_LIMIT_PER_EMAIL: Joi.number().integer().min(1).default(3),
  EOI_SUBMIT_LIMIT_PER_IP: Joi.number().integer().min(1).default(10),
