...
export class ApplicationsService {
  ...
  private async getInternshipByUuid(uuid: string): Promise<Internship> {
  ...
  async apply(
    ...
    const existing = await this.applicationModel.findOne({
      where: { internshipId: internship.id, studentId: student.id },
    });
    if (existing) {
  ...
  async findMine(userId: number, query: PaginationQueryDto) {
    ...
    const items = rows.map((row) => {
      const plain = row.get({ plain: true }) as Record<string, unknown> & {
        internship?: (Record<string, unknown> & { id: number; uuid: string }) | null;
      };
      const { internship, ...rest } = plain;
      return { ...rest, internship: internship ? serializeInternship(internship) : internship };
    });
  ...
  async findForInternship(internshipUuid: string, userId: number, query: QueryApplicationsDto) {
    ...
    const where: Record<string, unknown> = { internshipId: internship.id };
      ...
      include: [
        {
          ...
          // Email lives on the User row, not Student — the compact applicant
          // row shows it alongside phone, same as the full applicant-profile
          // view already did.
  ...
  async updateStatus(
    ...
    const plain = application.get({ plain: true }) as Record<string, unknown> & {
      internship?: (Record<string, unknown> & { id: number; uuid: string }) | null;
    };
    const { internship, ...rest } = plain;
    return { ...rest, internship: internship ? serializeInternship(internship) : internship };
  ...
  async getApplicantProfile(applicationId: number, user: AuthenticatedUser) {
    ...
    // Email is the one reliable contact channel every applicant has (unlike
    // LinkedIn/GitHub, which are optional) — worth the join for a screen
    // whose whole purpose is "does an employer have what they need to decide
    // and reach out."
