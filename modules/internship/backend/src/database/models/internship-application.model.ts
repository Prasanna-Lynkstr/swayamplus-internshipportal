...
@Table({
  ...
  // The (internshipId, studentId) unique index below only accelerates
  // internshipId-led lookups (leftmost prefix) — ApplicationsService.findMine()
  // filters by studentId alone on every "my applications" page load, so it
  // needs its own index rather than relying on that composite one.
  indexes: [{ fields: ['status'] }, { fields: ['studentId'] }],
})
