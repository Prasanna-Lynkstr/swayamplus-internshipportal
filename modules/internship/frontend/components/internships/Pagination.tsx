...
interface Props {
  ...
  /** Defaults to the internships browse page — pass buildCandidatesHref (or
   * any other Record<string,string|undefined>-keyed href builder) to reuse
   * this component elsewhere, e.g. the employer candidates directory. */
  buildHref?: (params: InternshipFilterParams, overrides: { page: string }) => string;
}

export function Pagination({ page, totalPages, currentParams, buildHref = buildInternshipsHref }: Props) {
  ...
  const hrefForPage = (p: number) => buildHref(currentParams, { page: String(p) });
