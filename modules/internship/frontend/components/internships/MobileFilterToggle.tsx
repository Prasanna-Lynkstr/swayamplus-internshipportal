...
export function MobileFilterToggle({
  ...
  return (
    // The sticky position has to live here, on the actual grid item (see
    // app/internships/browse/page.tsx's two-column grid) — applying it on
    // FilterSidebar's own <aside> instead doesn't work, because that
    // element sits nested a level deeper than the grid item itself once
    // this wrapper is in between, and a sticky element's containing block
    // needs to be the participant in the grid, not a descendant of it.
