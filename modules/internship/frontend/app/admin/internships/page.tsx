...
// useSearchParams (below) needs a Suspense boundary around the component
// that calls it — see next/docs use-search-params.md § Prerendering.
export default function AdminInternshipsPage() {
  return (
    <Suspense fallback={<p className="text-sp-ink-3">Loading…</p>}>
      <AdminInternshipsPageInner />
    </Suspense>
  );
}

function AdminInternshipsPageInner() {
  const { token } = useAuth();
